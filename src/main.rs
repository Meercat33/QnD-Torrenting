...
use std::{io::{Read, Write}, net::{self, TcpStream, Ipv4Addr, SocketAddr}};
use url::Url;
use errors::BTError;
...
fn split_http_from_bencoded_bytes(resp: &[u8]) -> Option<(&[u8], &[u8])> {
    let sep = b"\r\n\r\n";
...
fn get_peers<'a>(bencoded_data: Option<BencodeRef<'a>>) -> Option<Vec<u8>> {
...
fn peer_vec_to_list_of_ips(peer_vec: &mut Vec<u8>) -> Vec<SocketAddr> {
...
fn handle_http_tracker(tracker_url: &mut Url, torrent_file: Torrent) -> Result<TcpStream, BTError> {
...
fn handle_https_tracker(tracker_url: &mut Url, torrent_file: Torrent) -> Result<TlsStream<TcpStream>, BTError> {
    let info_hash = decode(torrent_file.info_hash()).expect("Invalid Hex");
    let info_hash = percent_encode(&info_hash, NON_ALPHANUMERIC);
